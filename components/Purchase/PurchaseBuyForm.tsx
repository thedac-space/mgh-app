...
const PurchaseBuyForm = ({ option }: { option: number }) => {
    ...
    const provider = useConnectWeb3().web3Provider
    ...
    useEffect(() => {
      console.log(provider,"PROVIDER")
        const getAllowance = async () => {
            ...
            const _provider = makeOwnProvider(purchaseCoinOptions[coin].chain)
    ...
    const transferToken = async () => {
        ...
        const amountToPay =
            monthlyChoice && amount * 10 ** purchaseCoinOptions[coin].decimals

        if (!amountToPay) return
        if (isERC20) {
            if (allowance < amountToPay) return

            if (coin == 'usdc') coinAddress = USDWallet
            else if (coin == 'usdt') coinAddress = USDWallet
            else if (coin == 'eth') coinAddress = ETHWallet
    ...
    const calculateAmoun = async () => {
        if (isERC20) {
            if (coin == 'usdc' || coin == 'usdt') coinAddress = USDWallet
            else if (coin == 'eth') coinAddress = ETHWallet

            setAmount(
                await contract?.purchaseRole(
                    [address, 1, 5, option],
                    coinAddress,
                    []
                )
    ...
    return (
        <>
            <div className="w-fit m-auto">
                {/* Show Amount */}
                <h3>
                    Total Amount: {amount} {coin?.toUpperCase()}
                </h3>
