...
const Stake: NextPage = () => {
    ...
    const { MGHBalance, allowance, totalStaked, rewards, totalSupply, APY } = useStakingContract(walletProvider, address, chainId)
    ...
    return (
        <>
            ...
            <div className="grid grid-flow-col grid-cols-1 lg:grid-cols-2 grid-rows-4 lg:grid-rows-2 gap-y-5 gap-x-20 max-w-5xl mt-8 xl:mt-0">
                ...
                <div className="flex flex-col space-y-5 items-center border-t border-l border-opacity-20 shadow-black rounded-xl p-5 w-full bg-grey-dark bg-opacity-30 max-w-4xl">
                    ...
                    <div className="flex flex-col items-center w-full max-w-sm">
                        ...
                        <div className="self-start flex items-center space-x-1 sm:space-x-3 w-full">
                            ...
                            <p className={`text-gray-300 font-medium text-lg pt-1`}>{(+totalSupply).toFixed(4)}</p>
                        </div>

                        <div className="self-start flex items-center space-x-1 sm:space-x-3 w-full">
                            <p className={`text-gray-300 font-medium text-lg pt-1 flex-grow`}>Current APY</p>
                            <p className={`text-gray-300 font-medium text-lg pt-1`}>{APY.toFixed(2)} %</p>
                        </div>

                        <div className="self-start flex items-center space-x-1 sm:space-x-3 w-full">
                            <p className={`text-gray-300 font-medium text-lg pt-1 flex-grow`}>Staked MGH</p>
                            <p className={`text-gray-300 font-medium text-lg pt-1`}>{(+totalStaked).toFixed(4)}</p>
                        </div>

                        <div className="self-start flex items-center space-x-1 sm:space-x-3 w-full">
                            <p className={`text-gray-300 font-medium text-lg pt-1 flex-grow`}>Rewards</p>
                            <p className={`text-gray-300 font-medium text-lg pt-1`}>{(+rewards).toFixed(4)}</p>
                        </div>

                    </div>

                    <button disabled={(+rewards) ? false : true} onClick={claimRewards} className={`disabled:opacity-50 disabled:hover:shadow-black disabled:cursor-default relative flex justify-center items-center border border-opacity-50 shadow-black hover:shadow-button transition ease-in-out duration-500 rounded-xl w-full max-w-sm py-4`}>
                        {/* <div className="h-full w-full absolute bg-gradient-to-br transition-all ease-in-out duration-300 from-pink-600 to-blue-500 rounded-xl blur-2xl group-hover:blur-xl" /> */}
                        <span className="pt-1 z-10 text-transparent bg-clip-text bg-gradient-to-br from-pink-600 to-blue-500 font-medium text-xl">Claim Rewards</span>
                    </button>
                </div>

                <div className="flex flex-col justify-between items-center border-t border-l border-opacity-20 shadow-black rounded-xl p-5 w-full bg-grey-dark bg-opacity-30 max-w-4xl">
                    <h3 className="text-gray-300 pb-0">Bridge</h3>

                    <p className="text-gray-300 font-medium max-w-sm">MGH staking is on Polygon to save network fees. For you to stake your $MGH, you have to first bridge them using the Polygon Bridge.</p>
