...
const ValuationPage: NextPage = ({ prices }: any) => {
    ...
    const [apiData, setAPIData] = useState<IAPIData | IAPIDataAxie>()
    ...
    const handleAPIData = (data: any) => {
        metaverse == 'axie-infinity' ?  null : (data = Object.values(data)[0])
        data.metaverse=metaverse
        setAPIData(data)

        const ethPrediction =
            metaverse == 'axie-infinity'
                ? data.prices.eth_predicted_price
                : data.floor_adjusted_predicted_price
            ...
            <div className="w-full flex flex-col items-center xs:w-[22rem] sm:w-full justify-start space-y-10 max-w-4xl mt-8 xl:mt-0">
                        ...
                        <div className="flex flex-col items-start gray-box">
                            ...
                            <div className="relative flex flex-wrap items-center mb-3 pl-1 text-left w-full max-w-sm">
                                ...
                                <p className="absolute -top-7 -left-6 xs:left-0 pl-2 px-2 py-1 rounded-lg bg-black bg-opacity-10 backdrop-filter backdrop-blur font-medium text-xs text-gray-400 hidden peer-hover:block w-70">
                                    Find LAND on Opensea &gt; Details &gt; Token
                                    ID
                                </p>
                            </div>
                            <form
                ...
                <div className="flex flex-col items-start shadow-blck rounded-xl py-3 px-4 w-full bg-grey-dark bg-opacity-20 text-left">
                    <p className={`text-xs sm:text-sm text-gray-400`}>
                        ...
                        recommendations or advisory services. Any information
                        provided through the land evaluation tool and others is
                        not, and should not be, considered as advice of any kind
                        and is for information purposes only. That land is
                        “valuated” does not mean, that it is in any way
                        approved, checked audited, and/or has a real or correct
                        value. In no event shall the MGH DAO be liable for any
                        special, indirect, or consequential damages, or any
                        other damages of any kind, including but not limited to
                        loss of use, loss of profits, or loss of data, arising
                        out of or in any way connected with the use of or
                        inability to use the Service, including without
                        limitation any damages resulting from reliance by you on
                        any information obtained from using the Service.
                    </p>
                </div>
            </div>
        </>
